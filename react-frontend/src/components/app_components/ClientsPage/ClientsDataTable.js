...
const ClientsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
<Column field="address" header="Address" body={pTemplate1} filter={selectedFilterFields.includes("address")} hidden={selectedHideFields?.includes("address")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Clients Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="clients"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Clients" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
